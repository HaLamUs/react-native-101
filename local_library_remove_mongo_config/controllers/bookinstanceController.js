...
// [JSON] Display list of all BookInstances
exports.bookinstance_list_json = function(req, res, next) {
  BookInstance.find()
    .populate("book")
    .exec(function(err, list_bookinstances) {
      if (err) {
...
//[JSON] Display detail page for a specific BookInstance
exports.bookinstance_detail_json = function(req, res, next) {
  BookInstance.findById(req.params.id)
    .populate("book")
    .exec(function(err, bookinstance) {
      if (err) {
...
//[JSON] Display BookInstance create form on GET
exports.bookinstance_lol = function(req, res, next) {
  console.log("bookinstance_create_get_json");
  Book.find({}, "title").exec(function(err, books) {
    if (err) {
      return next(err);
    }
...
exports.bookinstance_create_post = function(req, res, next) {
  console.log("receive req success", req.body); // ok
  req.checkBody("book", "Book must be specified").notEmpty(); // lấy cái id đủ rồi
  ...
  if (errors) {
    console.log("[bookinstance] err 1", errors);
    Book.find({}, "title").exec(function(err, books) {
      if (err) {
        return next(err);
      }
